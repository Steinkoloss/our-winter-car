...
Protocol version: **55** (`ProtocolInfo.Version` in `src/WinterMP.Net/Protocol.cs`).
...
| 25 | PlayerNeedsReport | 0 | guest -> host every ~12 s: playerId, hunger, fatigue, thirst, urine, **bodyTemp** (v28, `PLAYER/BodyTemp.Temperature` — the 5th need), **stress** (v30, `Stress` global — food/coffee/smoking move it), **drunk** (v30, `DrunkCurrent` on the FPS-camera "Drunk Mode" FSM), seq, then **dirtiness** (v55, `PlayerDirtiness`; appended after seq). The host stores only the authenticated player’s next finite report in the `wintermp-guests.json` sidecar (new rows have 16 columns; legacy 12–15-column rows retain missing trailing needs, and specifically omit dirtiness rather than falsely restoring clean). |
