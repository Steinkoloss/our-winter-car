...
Protocol version: **34** (`ProtocolInfo.Version` in `src/WinterMP.Net/Protocol.cs`).
...
| 56 | JobSiteState | 0 | host -> guests: stable site id, kind, flags (bit 0 active), sequence, primary and secondary values. Kind 1 sewage: `Called`, `ShitLevel`, `BasePrice`; kind 2 firewood: `Order`, `Surplus`, `Penalty`. Every registered house site is included in join snapshots. |
