...
Protocol version: **53** (`ProtocolInfo.Version` in `src/WinterMP.Net/Protocol.cs`).
...
| 86 | RallyState | 0 | host -> guests (v43): player id, SS stage (1-3), phase (idle/racing/finished), last accepted checkpoint, sequence and host-clocked elapsed centiseconds. Sent on each accepted crossing, every second while racing, and in join snapshots. |
