...
| 66 | CarRadioState | 0 | host -> guests (**v80**): seq, radioId (0 corris, 1 sorbet), channel (byte), volume (byte). Host owns the in-car radio channel + volume; guests apply. In the join snapshot. |
...
| 143 | PursuitState | 0 | host -> guests (**v72**): seq, flags (bit0/1 cop car 1/2 chasing, bit2/3 cop car 1/2 siren). Host owns the pursuit; guests apply the sirens (lights). Cop-car pose streams over NpcTransform. In the join snapshot. |
