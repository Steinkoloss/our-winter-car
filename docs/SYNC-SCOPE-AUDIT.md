...
| S08 | Installation, matching package, backup/restore, diagnostics and usable test distribution | Partial | Launcher installer, hashes/compatibility, backups and diagnostics exist. Local tester docs/manifest now track protocol261 with fresh-attempt package validation, not the historical protocol120 release. Payload-only output is explicitly not an installer; a full kit still requires the BepInEx vendor ZIP and current launcher/installer checks. Updater reads GitHub `/releases/latest`; it has no implemented beta/test-channel selector. Matching package/build evidence does not prove installation, Steam/two-PC or soak. |
...
| I10 | Beer-case bottle count and extraction; wood-carrier contents | Partial | Protocol261 adds a portable exact-case authority/absolute count and ItemWorldSync routing; unsafe generic `Remove bottle` replay is excluded. Native binding/input/count/save semantics remain unresolved, NOT native guest capability (see I10-BEER-CASE.md). Wood carriers still have native Woods/object arrays without a contents adapter. Shared motion alone does not reconstruct either container. |
