...
| Money/economy | **Single shared wallet** owned by host. All transactions are intents → host validates → broadcasts `WalletState`. v48 additionally requires every guest purchase intent to name a catalogued entry guard, advance that guest’s sequence, and originate from a fresh nearby pose before it can reach the host FSM. No race conditions by construction | ✅ |
