...
| Car assembly (bolts/parts) | Attach/detach + bolt-tightness as reliable events keyed to part IDs. Bolt-settle wear synced; **driving-induced part wear + breakage is now host/owner-authoritative** — engine breakage via owner-authoritative `VehicleDamage` (v60, non-owner RNG suppression), drivetrain wear + tire pressure/puncture via `VehicleCondition` (v61), selected gear in `VehicleState` (v63). | ✅ (v63; needs 2-player check) |
